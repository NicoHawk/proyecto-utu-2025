...
function agregarAutoAdmin($usuario, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual = 100) {
    if (empty($usuario)) {
        return ['exito' => false, 'mensaje' => 'Debe seleccionar un usuario'];
    }
    $autoModel = new Auto();
    $ok = $autoModel->insertarUsuario($usuario, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual);
    return ['exito' => (bool)$ok, 'mensaje' => $ok ? 'Auto agregado correctamente' : 'No se pudo agregar el auto'];
}

function editarAutoAdmin($id, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual = null) {
    $autoModel = new Auto();
    $ok = $autoModel->actualizarAdmin($id, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual);
...
function agregarAutoUsuario($usuario, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual = 100) {
    if (empty($usuario)) {
        return ['exito' => false, 'mensaje' => 'No autenticado'];
    }
    $autoModel = new Auto();
    $ok = $autoModel->insertarUsuario($usuario, $modelo, $marca, $conector, (int)$autonomia, (int)$anio, (int)$bateria_actual);
    return ['exito' => (bool)$ok, 'mensaje' => $ok ? 'Auto agregado' : 'No se pudo agregar'];
}

function editarAutoUsuario($usuario, $id, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual = null) {
    if (empty($usuario)) { return ['exito' => false, 'mensaje' => 'No autenticado']; }
    $autoModel = new Auto();
    $ok = $autoModel->actualizarUsuario((int)$id, $usuario, $modelo, $marca, $conector, (int)$autonomia, (int)$anio, $bateria_actual !== null ? (int)$bateria_actual : null);
    return ['exito' => (bool)$ok, 'mensaje' => $ok ? 'Auto actualizado' : 'No se pudo actualizar'];
}
...
if (basename($_SERVER['SCRIPT_FILENAME']) === 'AutoControlador.php') {
    ...
    switch ($accion) {
        case 'agregar':
            ...
            $bateria_actual = isset($data['bateria_actual']) ? (int)$data['bateria_actual'] : 100;
        ...
        case 'editar':
            ...
            $autonomia = isset($data['autonomia']) ? (int)$data['autonomia'] : 0;
            $anio      = isset($data['anio']) ? (int)$data['anio'] : 0;
            $bateria_actual = isset($data['bateria_actual']) ? (int)$data['bateria_actual'] : null;

            $ok = $autoModel->actualizarUsuario($id, $usuario, $modelo, $marca, $conector, $autonomia, $anio, $bateria_actual);
